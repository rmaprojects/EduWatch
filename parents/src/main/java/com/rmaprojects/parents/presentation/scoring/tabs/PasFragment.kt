...
import com.rmaprojects.parents.databinding.FragmentScorePasBinding
...
@AndroidEntryPoint
class PasFragment: Fragment(R.layout.fragment_score_pas) {

    private val viewModel: ScoringViewModel by viewModels()
    private val binding: FragmentScorePasBinding by viewBinding()

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        lifecycleScope.launch {
            viewModel.pasUiEvent.collectLatest { event ->
                when (event) {
                    ...
                    is ScoringUiEvent.Success -> {
                        ...
                        if (gradesList.isEmpty()) {
                            ...
                            binding.progressCircular.isVisible = false
                            binding.txtEmpty.isVisible = false
                            binding.rvScorePas.adapter = SubjectListAdapter(
                                viewModel.pasGradesList,
                                null
                            )
                        }
                    }
